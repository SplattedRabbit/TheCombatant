 ...
 * @notHere   Offensive Angriffswerte ÔåÆ PCOffense.js | Fertigkeiten ÔåÆ PCSkillsTab.js | Talente ÔåÆ PCFeatsTab.js
 */
...
export function renderPCAttributes(pc) {
  ...
  const renderAttrBox = (key, label, icon) => {
    ...
    if (hasModifiers) {
      ...
      tooltip += `\nBasiswert: ${stat.base}\nAktiver Wert: ${score}\nAktive Boni:\n` + 
        modifiers.map(m => `ÔÇó ${m.source}: ${formatMod(m.value)}`).join('\n');
    }
    ...
    return `
      <div class="attr-box" style="display:flex; flex-direction:column; ${bgStyle} ${borderStyle} border-radius:2px; padding:3px; position:relative;" title="${tooltip}">
        ...
        <div style="display:flex; align-items:center; gap:2px; margin-top:2px; justify-content:space-between;">
          ...
          <button class="xbtn roll-attr-btn" data-attr="${key}" style="padding:0; width:16px; height:14px; font-size:8px; line-height:14px; display:flex; align-items:center; justify-content:center;" title="${label}wurf (Formel)">­ƒÄ▓</button>
        </div>
      </div>
    `;
  };
  ...
  attributes.innerHTML = `
    <div class="phdr"><h2>Ô£¿ Attribute &amp; Kompetenz</h2></div>
    <div class="pbody" style="display:flex; flex-direction:column; gap:5px;">
      ...
      <div style="background:rgba(139,26,26,0.04); border:0.5px solid rgba(139,26,26,0.15); border-radius:2px; padding:5px 6px; margin-bottom:4px; display:flex; flex-direction:column; gap:4px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom: 0.5px solid rgba(139,26,26,0.1); padding-bottom: 4px; margin-bottom: 2px;">
          <span style="font-family:'IM Fell English SC', serif; font-size:9px; color:var(--red); font-weight:600; letter-spacing:0.3px;">­ƒº¼ Volk / Rasse</span>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
          <span style="font-family:'IM Fell English SC', serif; font-size:9px; color:var(--red); font-weight:600; letter-spacing:0.3px;">­ƒÄ¡ Klassen &amp; Stufen</span>
          ${classesCount < 4 ? `
            ...
            return `
              <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(200, 169, 110, 0.1); border:0.5px solid var(--pb); border-radius:1px; padding:2px 4px; font-size:8px;">
                ...
                <div style="display:flex; align-items:center; gap:4px;">
                  ...
                  <button class="xbtn xbtn-del btn-remove-class" data-idx="${idx}" style="padding:0 3px; font-size:7.5px; height:13px; line-height:11px;" title="Klasse entfernen">Ô£ò</button>
                </div>
              </div>
            `;
          }).join('') : `
        ...
        <div id="addClassForm" style="display:none; flex-direction:column; gap:3px; background:rgba(200,169,110,0.15); border:0.5px solid var(--pb); border-radius:1.5px; padding:3px; margin-top:2px;">
          ...
          <div style="display:flex; justify-content:flex-end; gap:2px;">
            <button class="btn btn-p" id="btnAddClassConfirm" style="font-size:7px; padding:1px 5px;">Hinzuf├╝gen</button>
            <button class="btn" id="btnAddClassCancel" style="font-size:7px; padding:1px 5px;">Ô£ò</button>
          </div>
        </div>
      </div>

      <div style="display:grid; grid-template-columns: 1fr 1fr; gap:4px;">
        ${renderAttrBox('str', 'St├ñrke', 'ÔÜö´©Å')}
        ${renderAttrBox('dex', 'Geschick', '­ƒÄ»')}
        ${renderAttrBox('con', 'Konstitution', '­ƒøí´©Å')}
        ${renderAttrBox('int', 'Intelligenz', '­ƒºá')}
        ${renderAttrBox('wis', 'Weisheit', '­ƒö«')}
        ${renderAttrBox('cha', 'Charisma', 'Ô£¿')}
      </div>
      <div style="display:flex; justify-content:space-between; align-items:center; margin-top:3px; background:rgba(139,26,26,0.05); border:0.5px solid rgba(139,26,26,0.2); border-radius:2px; padding:4px;">
        <label style="font-size:9px; font-weight:bold; color:var(--red);">ÔÜö´©Å Basisangriff (BAB):</label>
  ...
  attributes.querySelectorAll('.btn-remove-class').forEach(btn => {
    btn.onclick = () => {
      ...
      showCustomConfirm(
        "Klasse entfernen? ÔÜá´©Å", 
        `M├Âchtest du die Klasse "${className}" wirklich entfernen? Dadurch werden alle zugeh├Ârigen Klassendaten und Sonderf├ñhigkeiten zur├╝ckgesetzt, um Anzeigefehler zu vermeiden.`, 
        () => {
  ...
  attributes.querySelectorAll('.roll-attr-btn').forEach(btn => {
    btn.onclick = (e) => {
      ...
      const mod = getAblMod(score);
      const labels = { str: 'St├ñrke', dex: 'Geschick', con: 'Konstitution', int: 'Intelligenz', wis: 'Weisheit', cha: 'Charisma' };
